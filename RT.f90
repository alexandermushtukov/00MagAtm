...
!
! Lower boundary:
!   Tbot  = temperature of the lower boundary / ghost point n_m
!   Tprev = temperature of the last real atmospheric point n_m-1
!   Their difference determines the diffusion flux entering the atmosphere.
!==========================================================================================================================
...
real*8 :: map_frac(n_mu,n_mu,n_fi,n_fi)
    ...
    do while(j.le.n_mu)
      ...
      do while(kk.le.n_fi)
        ...
        do while(k.le.n_fi)
          ...
          call get_index_for_R(q1,q2,qq1,qq2,frac, &
                               jj,j,kk,k,m_coord_b,n_mu,n_fi,dmu,dfi)
      ...
      do while(i_pol.le.2)
        ...
        if( mu.ge.0.d0 )then
          ...
          do while(ii.le.n_m)
            ...
            kappa = m_atm_kappa(ii,j,k,1,i_pol) + &
                    m_atm_kappa(ii,j,k,2,i_pol)

            dtau = dSigma * kappa / abs(mu)

            if( dtau.ne.0.d0 )then

              A_up(ii) = S_0(ii,j,k,i_pol)/kappa * &
                         ( 1.d0 - exp(-dtau) )
          ...
          !== Diffusion-type lower boundary condition.
          !   The temperature gradient between n_m-1 and n_m
          !   is controlled externally by the required flux. ==!

          Bbot  = 0.5d0 * BB_Intensity_22(E,Tbot)
          Bprev = 0.5d0 * BB_Intensity_22(E,Tprev)

          dm_bot = max( mas_m_rho(n_m,1) - &
                        mas_m_rho(n_m-1,1), 1.d-30 )

          dBdm = (Bbot - Bprev) / dm_bot

          !== opacity at the lower boundary;
          !   average over the last real point and the boundary point ==!
          kappa = 0.5d0 * &
                  ( m_atm_kappa(n_m,j,k,1,i_pol)   + &
                    m_atm_kappa(n_m,j,k,2,i_pol)   + &
                    m_atm_kappa(n_m-1,j,k,1,i_pol) + &
                    m_atm_kappa(n_m-1,j,k,2,i_pol) )

          kappa = max(kappa,1.d-30)

          dBdTauNu = dBdm / kappa

          !== incoming intensity from the deeper atmosphere ==!
          ...
          do while(ii.le.n_m)
            ...
            if( dtau.ne.0.d0 )then
              ...
              A_dn(ii) = S_0(ii,j,k,i_pol)/kappa * &
                         ( 1.d0 - exp(-dtau) )
          ...
          !== zero incoming radiation at the upper boundary ==!
          I_dn(1) = A_dn(1)
  ...
  !== get new source function, in units [kappa*B_22] ==!
  S(1:n_m,1:n_mu,1:n_fi,1:2) = 0.d0
      ...
      do while( k.le.n_fi )
        ...
        do while(jj.le.n_mu)
          ...
          do while(kk.le.n_fi)
            ...
            RR(1:2,1:2) = &
                 (1.d0-frac)*R_b(i,q1,q2,qq1,1:2,1:2) + &
                          frac *R_b(i,q1,q2,qq2,1:2,1:2)

            S(i,j,k,1) = S(i,j,k,1) + &
                         ( RR(1,1)*I_e(i,jj,kk,1) * &
                           m_atm_kappa(i,jj,kk,2,1) &
                         + RR(2,1)*I_e(i,jj,kk,2) * &
                           m_atm_kappa(i,jj,kk,2,2) ) * dmu*dfi

            S(i,j,k,2) = S(i,j,k,2) + &
                         ( RR(1,2)*I_e(i,jj,kk,1) * &
                           m_atm_kappa(i,jj,kk,2,1) &
                         + RR(2,2)*I_e(i,jj,kk,2) * &
                           m_atm_kappa(i,jj,kk,2,2) ) * dmu*dfi
